...
def create_monthly_rent_df(df):
    ...
    monthly_rent_df["dteday_str"] = monthly_rent_df["dteday"].dt.strftime(
        '%Y-%m')  # Buat kolom string
...
def create_seasonal_rent_df(df):
    seasonal_rent_df = day_df.groupby(
...
ax.set_title("Hari dengan Sewa Sepeda Paling Banyak dan Paling Sedikit",
...
ax.plot(monthly_rent_df["dteday"], monthly_rent_df["cnt"],
        marker='o', linewidth=2, color="#72BCD4")
ax.set_title("Tren Sewa Sepeda dalam Beberapa Bulan Terakhir",
...
ax.set_title(
    "Perbandingan Jumlah Penyewa Sepeda: Casual vs. Registered", fontsize=10)
st.pyplot(fig)

st.write("### Seasonal Rentals")
seasonal_rent_df = main_day_df.groupby(
    "season")[["casual", "registered", "cnt"]].sum().reset_index()
